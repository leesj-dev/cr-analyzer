...
export function FilterCombobox({ allCards, selectedCards, onSelectionChange }: Props) {
  ...
  const koreanFilter = (value: string, search: string): number => {
    const searchLower = search.toLowerCase();
    const koreanLabel = translateCardName(value).toLowerCase();
    if (koreanLabel.includes(searchLower)) {
      return 1;
    }
    return 0;
  };
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
                ...
                <CommandGroup>
                  {options.map((option) => (
                    <CommandItem key={option.value} value={option.value} onSelect={handleSelect}>
                      <Check className={cn("mr-2 h-4 w-4", selectedCards.includes(option.value) ? "opacity-100" : "opacity-0")} />
                      {option.label}
                    </CommandItem>
                  ))}
                </CommandGroup>
        ...
        <Button variant="ghost" size="icon" onClick={() => onSelectionChange([])} disabled={selectedCards.length === 0}>
